...
1. ESLint支持最新的JavaScript语法特性。此外，ESLint支持插件机制，这也意味着用户可以创建和共享能够被添加到ESLint配置中的插件。

1. 测试对于Redux至关重要，因为它只和数据打交道，它并不包含UI。Redux天生就是可测试的

1. 测试驱动开发（TDD）是一种实践而非技术。这并不意味着用户只需让自己的应用程序拥有测试代码即可。相反，它是一种测试驱动的开发过程的时间。

    1. 先写测试是最关键的一步。用户在一个测试中声明将要研发的内容和他的工作原理。

    1. 运行测试并观察它们的失误

    1. 编写最少量的必需代码，让它们通过测试，重点放在让每个测试通过上面，并且不要添加任何超过测试范围的功能。

    1. 重构上述程序代码和测试用例，一旦测试通过，那么就是该认真检查项目代码和测测试用例的时候了，尽量将项目代码变得简洁优雅。

1. TDD是处理Redux应用的绝佳方式

1. 在你成为TDD老手之前，在测试之前编写项目代码没有什么问题。先尝试一些小批量的测试：一小段代码，几个测试用例等。

1. Reducer是根据输入参数进行一系列计算并返回结果的纯函数。在一个测试中，我们可以控制输入、当前State和Action。给定一个当前的State和Action，我们将能预计一个Reducer的输出结果。

1. Jest是为React量身定制的JavaScript测试框架，jest将会运行tests文件中的任何测试，以及用户项目文件中以 .test.js 后缀结尾的任何JavaScript文件。

1. jest将会为每条错误提示提供具体细节，包括堆栈跟踪

1. 一个典型的TDD周期。我们首先编写测试代码，然后编写可以通过测试的项目代码，最后对项目代码和测试代码进行重构。在使用JavaScript工作时，该方法特别高效，特别对于Redux来说

1. jest 集成了一个名为jsdom的npm软件包，它可以用于在Nodjs中模拟浏览器环境，这对于React组件至关重要

### React Router

1. 在一个单页应用中，所有的内容都是在同一页面呈现的。JavaScript负责载入信息和修改UI。

1. 浏览器的历史纪录、书签、前进和回退功能在没有路由解决方案的情况下将无法正常工作。

1. 所有的Route组件都需要包装成一个router，在这种情况下，HashRouter将会根据窗体地址栏上的地址渲染显示相关组件。Link组件可以方便导航。

1. Route组件可以用于匹配特定URL地址后才显示相关内容。这个特性允许用户将Web应用组织成一定的层级结构，从而有利于内容复用。

1. 将路由包装到一个Switch组件中，可以确保只渲染一个Colors容器。

1. 路由参数是获取影响用户界面表现层数据的理想工具。不过只有当开发人员希望用户通过URL获取这些细节时才应该被采用。

1. 地址栏中的#是为了阻止用户发送get请求的

### React服务端应用

1. 同构性应用程序是可以在多个平台上渲染的程序。通用性代码意味着完全相同的代码可以在多中环境下运行。

1. 使用ReactDOM.renderToString方法允许用户在服务器端渲染ui。服务器的功能非常强大，它能够访问所有资源的能力是浏览器所不具备的。服务器可以非常安全，并且可以安全的访问数据。用户可以在服务端渲染初始内容的优势之上充分利用这些额外的优点。

1. Redux库中的所有JavaScript代码都是通用的。用户使用JavaScript编写的Reducer也不应该包含任何与执行环境有关的代码。